#created by vignesh on 13/11/2017
...
class Generator_model(nn.Module):
	...
	self.conv_final=nn.Conv2d(64,3,1,1,padding=0)
    ...
    def decoder(self):
        ...
        self.deconv4 = nn.ConvTranspose2d(512*2,512,4,2,padding=1)
        self.deconv4_bn = nn.BatchNorm2d(512)
        
        #512+256,32,32 - 256,64,64
        self.deconv3 = nn.ConvTranspose2d(512+256,256,4,2,padding=1)
        self.deconv3_bn = nn.BatchNorm2d(256)
        
        #256+128,64,64 -128,128,128 
        self.deconv2 = nn.ConvTranspose2d(256+128,128,4,2,padding=1)
        self.deconv2_bn = nn.BatchNorm2d(128)
        
        #128+64,128,128 - 64,256,256
        self.deconv1 = nn.ConvTranspose2d(128+64,64,4,2,padding=1)
...
class Discriminator_Model(nn.Module):
        def __init__(self):
            ...
            self.conv1 = nn.Conv2d(3+1,64,4,2,padding=0)
            
            self.conv2= nn.Conv2d(64,128,4,2,padding=0)
            self.conv2_bn = nn.BatchNorm2d(128)
            
            self.conv3= nn.Conv2d(128,256,4,2,padding=0)
            self.conv3_bn = nn.BatchNorm2d(256)
            
            self.conv4= nn.Conv2d(256,512,4,1,padding=0)
            self.conv4_bn = nn.BatchNorm2d(512)
            
            self.conv5= nn.Conv2d(512,512,4,1,padding=0)
            self.conv_final=nn.Conv2d(512,1,21,1,padding=0)
        ...
        def forward(self,input,output):
            ...
            a5=self.conv4_bn(self.conv5(a4_activ))
	    a5_activ=F.leaky_relu(a5,0.2)
	    a6=self.conv4_bn(self.conv5(a5_activ))
	    a6_activ=F.leaky_relu(a5,0.2)
	   
            logits=self.conv_final(a6_activ)
            return nn.functional.tanh(logits)
...
class losses():
    ...
    def dloss(self,real,fake):
        ...
        return F.binary_cross_entropy(F.sigmoid(real),self.ones)+F.binary_cross_entropy(F.sigmoid(fake),self.zeros)
        
    def gloss(self,G_fake,D_fake,color):
        return F.binary_cross_entropy(F.sigmoid(D_fake),self.ones)+100*F.smooth_l1_loss(G_fake,color)
